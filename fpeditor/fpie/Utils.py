...
class Utils:
    @staticmethod
    def show_image(image, label, vmin=0.0, vmax=1.0):
        plt.figure().suptitle(label)
        plt.imshow(image, cmap="gray", vmin=vmin, vmax=vmax)

    @staticmethod
    def show_orientations(image, orientations, label, w=32, vmin=0.0, vmax=1.0):
    ...
    @staticmethod
    def draw_image(source, destination, y, x):
    ...
    @staticmethod
    def local_normalize(image, w=32):
        image = np.copy(image)
        height, width = image.shape
        for y in range(0, height, w):
            for x in range(0, width, w):
                image[y:y+w, x:x+w] = Utils.normalize(image[y:y+w, x:x+w])
    ...
    def convolve(image, kernel, origin=None, shape=None, pad=True):
        ...
        kernel_origin = (-k.shape[0] // 2, -k.shape[1] // 2)
        ...
        if pad:
            top_padding = max(0, -(origin[0] + kernel_origin[0]))
            left_padding = max(0, -(origin[1] + kernel_origin[1]))
            bottom_padding = max(0, (origin[0] + shape[0] + kernel_origin[0] + kernel_shape[0]) - image.shape[0])
            right_padding = max(0, (origin[1] + shape[1] + kernel_origin[1] + kernel_shape[1]) - image.shape[1])
        ...
        for y in range(shape[0]):
            for x in range(shape[1]):
                iy = top_padding + origin[0] + y + kernel_origin[0]
                ix = left_padding + origin[1] + x + kernel_origin[1]

                block = image[iy:iy+kernel_shape[0], ix:ix+kernel_shape[1]]
                if callable(kernel):
    ...
    @staticmethod
    def find_mask(image, threshold=0.1, w=32):
        """
    ...
    @staticmethod
    def average_orientation(orientations, weights=None, deviation=False):
        """
    ...
    @staticmethod
    def average_frequency(frequencies):
        """
    ...
    @staticmethod
    def estimate_orientations(image, w=16, interpolate=True):
        """
        ...
        G_x = Utils.convolve(image, Utils.sobel_kernel_x())
        G_y = Utils.convolve(image, Utils.sobel_kernel_y())
        ...
        for y in range(yblocks):
            for x in range(xblocks):
                ...
                orientation, deviation = Utils.average_orientation(surrounding, deviation=True)
                if deviation > 0.5:
        ...
        if interpolate:
            ...
            for y in range(yblocks - 1):
                for x in range(xblocks - 1):
                    for iy in range(w):
                        for ix in range(w):
                            orientations[y*w+hw+iy, x*w+hw+ix] = Utils.average_orientation(
    ...
    def estimate_frequencies(image, orientations, w=32):
        ...
        for y in range(yblocks):
            for x in range(xblocks):
                ...
                block = Utils.rotate_and_crop(block, np.pi * 0.5 + orientation)
                if block.size == 0:
                    F[y, x] = -1
                    continue

                Utils.draw_image(block, rotations, y * w, x * w)
